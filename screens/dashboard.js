...
function individualCompliance(ind) {
  ...
  const latestScr = (S.screenings || [])
    .filter(s => s.individualId === ind.individualId)
    .sort((a, b) => b.date?.localeCompare(a.date))[0];

  const latestTrn = (S.training || [])
    .filter(t => t.individualId === ind.individualId)
    .sort((a, b) => b.completedDate?.localeCompare(a.completedDate))[0];

  const latestVet = (S.vetting || [])
    .filter(v => v.individualId === ind.individualId)
    .sort((a, b) => b.policeCheckDate?.localeCompare(a.policeCheckDate))[0];
...
function firmGaps() {
  ...
  if (!firm.austracEnrolment?.enrolmentId) {
    gaps.push({
      label: 'AUSTRAC enrolment ID not recorded',
      screen: 'firm-profile-edit',
      params: { tab: 'enrolment' },
    });
  }

  if (!firm.amlProgram?.version) {
    gaps.push({
      label: 'AML/CTF Program approval not recorded',
      screen: 'firm-profile-edit',
      params: { tab: 'program' },
    });
  }

  if (!firm.riskAssessment?.rating) {
    gaps.push({
      label: 'Firm risk assessment not recorded',
      screen: 'firm-profile-edit',
      params: { tab: 'risk' },
    });
  }

  if (!firm.designatedServices?.length) {
    gaps.push({
      label: 'Designated services not recorded',
      screen: 'firm-profile-edit',
      params: { tab: 'services' },
    });
  }
...
export function screen() {
  ...
  const upcoming = entities
    .filter(e =>
      e.riskNextReviewDate &&
      new Date(e.riskNextReviewDate) <= in90 &&
      new Date(e.riskNextReviewDate) >= today
    )
  ...
  return `
    <div>
      ...
      <!-- Beta / reliance notice -->
      <div class="banner banner-info" style="margin-bottom:var(--space-4);">
        SimpleAML Pro helps you record and review AML/CTF compliance information.
        It does not provide legal advice or guarantee compliance. Your firm remains responsible
        for reviewing its obligations and keeping appropriate supporting records.
      </div>
      ...
      <div style="background:${verdict.bg};border:0.5px solid ${verdict.border};border-radius:var(--radius-xl);padding:var(--space-5) 22px;margin-bottom:var(--space-4);display:flex;align-items:center;justify-content:space-between;">
        <div>
          <div style="font-size:var(--font-size-xs);color:${verdict.color};font-weight:var(--font-weight-medium);text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px;">
            Records status overview
          </div>
          <div style="font-size:22px;font-weight:var(--font-weight-medium);color:${verdict.color};">
            ${verdict.label}
          </div>
          ${totalIssues > 0 ? `
            <div style="font-size:var(--font-size-xs);color:${verdict.color};margin-top:4px;">
        ...
        <div style="text-align:right;">
          <div style="font-size:28px;font-weight:var(--font-weight-bold);color:${verdict.color};">
            ${recordsComplete.length}/${individuals.length}
          </div>
          <div style="font-size:var(--font-size-xs);color:${verdict.color};">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:var(--space-3);margin-bottom:var(--space-4);">
        ${[
          ...
          >
            <div style="font-size:24px;font-weight:var(--font-weight-bold);color:${s.color};margin-bottom:4px;">
              ${s.value}
            </div>
            <div style="font-size:var(--font-size-xs);color:var(--color-text-muted);">
              ${s.label}
      ...
      <div style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:var(--space-4);">
        ...
        <div>
              ...
              <div style="margin-top:var(--space-3);padding:var(--space-2) 0;border-top:0.5px solid var(--color-border);">
                <div style="display:flex;align-items:center;gap:var(--space-2);">
                  ...
                  <span style="font-size:var(--font-size-xs);color:var(--color-text-primary);">
                    AML/CTF Program review due ${fmtDate(progReviewDate)}
                  </span>
                </div>
              </div>
            ` : ''}
          ...
          ${action.length > 0 ? `
            <div class="card">
              <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--space-3);">
                ...
                <button onclick="go('staff',{filter:'action'})" class="btn-ghost" style="font-size:var(--font-size-xs);color:var(--color-primary);">
                  View all
                </button>
              </div>
              ...
              ${action.slice(0, 5).map(r => `
                ...
                >
                  ...
                  <div style="flex:1;min-width:0;">
                    ...
                    <div style="font-size:10px;color:var(--color-danger);">
                      ${r.result.missing[0]?.label || 'Record incomplete'}
                    </div>
                  </div>
          ...
          ${noLinks.length > 0 ? `
            <div class="card">
              <div class="section-heading">Records without active roles</div>
              <p style="font-size:var(--font-size-xs);color:var(--color-text-secondary);line-height:var(--line-height-relaxed);">
                ${noLinks.length} individual record${noLinks.length === 1 ? '' : 's'} currently have no active staff or client role.
                Review whether these records are still needed or should be linked to a firm/client role.
              </p>
            </div>
          ` : ''}
        ...
        <div>
          ...
          <div class="card">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--space-3);">
              ...
              <button onclick="go('clients')" class="btn-ghost" style="font-size:var(--font-size-xs);color:var(--color-primary);">
                All clients
              </button>
            </div>
            ...
            ${upcoming.length === 0 ? `
              <p style="font-size:var(--font-size-xs);color:var(--color-text-muted);">
                No client reviews due in the next 90 days.
              </p>
            ` : upcoming.map(e => {
          ...
          <div class="card">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--space-3);">
              ...
              <button onclick="go('audit-trail')" class="btn-ghost" style="font-size:var(--font-size-xs);color:var(--color-primary);">
                Audit trail
              </button>
            </div>
            ...
            ${recent.length === 0 ? `
              <p style="font-size:var(--font-size-xs);color:var(--color-text-muted);">
                No individuals yet. Add your first staff member or client to get started.
              </p>
            ` : recent.map(i => `
              <div
